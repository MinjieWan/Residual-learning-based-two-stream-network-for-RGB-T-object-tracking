...
import pickle
import time
sys.path.append('/home/a11134315/MANet-master-change/modules')
import torch
...
img_home = '/home/test/chenyili/RGB_T234'
data_path1 = '/home/test/chenyili/MANet-master-change/DATA/rgbt234_v.pkl'
data_path2 = '/home/test/chenyili/MANet-master-change/DATA/rgbt234_i.pkl'
#*********************************************************************************************************
...
def train_mdnet():
    ...
    for i in range(opts['n_cycles']):
        ...
        for j,k in enumerate(k_list):
            ...
            pos_score,pos_score_t,pos_score_r = model(pos_regions1,pos_regions2 ,k)
            neg_score,neg_score_t,neg_score_r = model(neg_regions1,neg_regions2, k)
            loss_t = criterion(pos_score_t, neg_score_t)
            loss_r = criterion(pos_score_r, neg_score_r)
            loss = criterion(pos_score, neg_score)
            punish = max(loss.item() - loss_r.item(), loss.item() - loss_t.item(), 0)
            print (loss.item(), loss_t.item(), loss_r.item(), punish)
            loss = loss + punish
            loss1[k]=loss
            model.zero_grad()
            loss.backward()
