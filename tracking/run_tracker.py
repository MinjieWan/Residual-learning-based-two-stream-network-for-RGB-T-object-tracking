...
sys.path.append('/home/test/chenyili/MANet-master-change/modules')
import torch
...
sys.path.append('/home/test/chenyili/MANet-master-change/tracking')
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, in_layer='fc4'):
    ...
    for iter in range(maxiter):#训练maxiter轮
...
def run_mdnet(img_list1,img_list2, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    if display or savefig: 
        dpi = 80.0
        figsize = (image2.size[0]/dpi, image2.size[1]/dpi)
        # figsize2=(image2.size[0]/dpi,image2.size[1]/dpi)
        fig = plt.figure(frameon=False, figsize=figsize, dpi=dpi)
        # fig2=plt.figure(frameon=False,figsize=figsize2,dpi=dpi)
        ...
        fig.add_axes(ax)
        # fig2.add_axes(ax2)
        im = ax.imshow(image1, aspect='auto')
        # im2=ax.imshow(image2,aspect='auto')
        if gt is not None:
        ...
        if display:
            ...
            plt.draw()
        if savefig:
            # fig1=np.concatenate((fig,fig2),axis=1)
            fig.savefig(os.path.join(savefig_dir,'0000.jpg'),dpi=dpi)
            # fig2.savefig(os.path.join(savefig_dir,'infraed','0000.jpg'))
...
def get_sequence(seq,seq_home):
    result_home='/home/test/chenyili/MANet-master-change/tracking_result/MANet-final1-RGBT_234_result' # you need creat result dirpath
    ...
    init_bbox=gt[0]
    savefig_dir=os.path.join(result_home,'{}'.format(seq))
    result_dir=os.path.join(result_home)
    if not os.path.exists(result_dir):
        os.makedirs(result_dir)
    if not os.path.exists(savefig_dir):
        os.makedirs(savefig_dir)
    result_path=os.path.join(result_dir,'MANet_'+seq+'.txt')
    return img_list1,img_list2,init_bbox,gt,result_path,savefig_dir

data_dir='/home/test/chenyili/RGB_T234' # set tracking dataset path

res_dir='/home/test/chenyili/MANet-master-change/tracking_result/MANet-final1-RGBT_234_result' # you need creat result dirpath
...
if __name__ == "__main__":
    ...
    list_path=os.path.join('/home/test/chenyili/MANet-master-change/DATA/rgbt234.txt')
    ...
    for i in range(n_seq):
        ...
        seq=seqs[i]
        print(i,seq)
        if 'MANet_'+seq+'.txt' not in os.listdir(res_dir):
            np.random.seed(123)
            torch.manual_seed(456)
            torch.cuda.manual_seed(789)
        
            img_list1,img_list2,init_bbox,gt,result_path,savefig_dir=get_sequence(seq,data_dir)

            # Run tracker
            result, result_bb, fps = run_mdnet(img_list1, img_list2,init_bbox, gt=gt, savefig_dir=savefig_dir, display=False)
            print('FPS==>',fps)
